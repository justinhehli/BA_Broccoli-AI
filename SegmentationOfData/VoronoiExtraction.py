...
#set tp tems connection
myTeamsMessage = pymsteams.connectorcard("https://outlook.office.com/webhook/3e6f8e6b-554f-4c7c-85f7-a59b46f00ab8@de095962-16fc-4403-92df-52d8a429fa05/IncomingWebhook/0a3fa65c0fcd45b1b6cdcc6890a75772/3cd44d59-7db7-4754-8070-0bd38b83d6dc")
...
if(useCSV == False):
    ndvi_src_string = r'\\fs004\ice\lehre\bachelorarbeiten\2019\Pflanzen\Drohnenaufnahmen\20190613\export\20190613RMC_index_ndvi_modified.tif'
    ndre_src_string = r'\\fs004\ice\lehre\bachelorarbeiten\2019\Pflanzen\Drohnenaufnahmen\20190613\export\20190613RMC_index_ndre_modified.tif'
...
for index, row in data.iterrows():
    try:
        ...
        myTeamsMessage.title("Iteration of measurement date: {}".format(timestamp))
        myTeamsMessage.text("NDVI Tif file: " + ndvi_src_string)
        myTeamsMessage.text("NDRE Tif file: " + ndre_src_string)
        myTeamsMessage.send()
        ...
        if Data2SQL == True:
            try:
                myTeamsMessage.text("Values of date {} will be added to SQL database".format(timestamp))
                myTeamsMessage.send()
                subprocess.call([r"Data2SQL\Data2SQL\bin\Debug\Data2SQL.exe", output_src_string+'\export.csv'])
                myTeamsMessage.text("Values of date {} were successfully added to SQL database".format(timestamp))
                myTeamsMessage.send()
            except Exception as e:
                myTeamsMessage.title("FAILED on measurement: {}".format(timestamp))
                myTeamsMessage.text("Error while inserting data to SQL database, Script will quit")
                myTeamsMessage.text("Stacktrace: " + e)
                myTeamsMessage.send()
                exit()
    except Exception as e:
        myTeamsMessage.title("FAILED on measurement: {}".format(timestamp))
        myTeamsMessage.text("Error while doing the segmentation, Script will quit")
        myTeamsMessage.text("Stacktrace: " + e)
        myTeamsMessage.send()    
        exit()